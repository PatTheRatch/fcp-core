...
**Companions:** [`injuries.md`](injuries.md) (the reports, the schema and the point-in-time rule), [`projection_prior.md`](projection_prior.md) (the checkpoint design this reuses), [`pickups.md`](pickups.md) §4 (the availability discount)
...
- **Nothing is wired and nothing reads this.** It is a measurement: no file in `app/` changed, the product's 0.881 is untouched, and the branch adds one script and one document.
