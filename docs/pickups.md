...
**As built, fourth pass** (2026-09-21, `app/pickups/state.py`): `my_totals` and `opp_totals` take the day. `matchup_team_stats` has no day column, so for a period the season has run past it is the **final** total, not the running one; a report for a replayed day therefore opened with the whole week already banked and projected the days still to come on top of it (docs/inseason_rehearsal.md, finding 1: 709 points posted on the morning of the first day, a projected 1560.9). `_posted` now keeps ESPN's own row only when the database holds no `player_game_stats` on or after `today` — the one case where that row is the running tally, and where its stat corrections are worth more than our own sum — and otherwise sums the started lines on the period's days **before** `today`. Exclusive, because `scoring_periods_remaining` begins at `today` and the projection adds that day itself; verified against ESPN on nine 2026 team-periods, where the same sum over a whole period reproduces `matchup_team_stats` exactly. `scripts/pickups_backtest.py` no longer substitutes its own version.
...
**Hurdle**: recommend only when the net over both horizons ≥ 0.20 categories (0.10 until the 2026 backtest, docs/pickups_backtest.md), or an empty day is filled. Below that, say so.
...
**As rebuilt** (2026-09-24, [`stash_mode.md`](stash_mode.md)). Two sentences above were wrong and the lane could not fire because of the first. **ESPN's basketball API carries no `expectedReturnDate` at all** — not an empty one; the field does not exist on the player object in either kona view, confirmed by a read-only probe on 1,097 entries of which 136 were OUT — so the gate above was empty on every call it ever had and `_stashes` returned `()` for two years. It now asks the return prior of [`stashes.md`](stashes.md) §2a how long the place is expected to stand empty, which is what the date was standing in for; when a date does arrive, the date wins. And **the IR slot is read from the stored setting, never from this sentence**: `league_seasons.injured_reserve_slots` for 2027 is **0**, not 1 (`stashes.md` §5), and `TeamWeek.ir_slot_free` against the roster's own occupancy is what decides whether a stash costs a place. Beside each candidate goes the block the what-if and both sides of a trade carry: days out, the odds of being back by weeks 1, 2, 4 and 8, the expected dead weeks, what they cost at `OPENED_PLACE`, and the net either side of them.
...
**As built** (2026-09-23, [`docs/faab.md`](faab.md)): the market number above is only half a bid, and it is the half a competitor can compute too. The module now carries three things beside it -- what the move is worth to *this* roster in dollars, at the shadow price of this manager's own budget; the dollar above which it stops clearing his bar; and `P(win | bid $x)` per rank bucket, read off the losing claims ESPN keeps (`FAILED_INVALIDPLAYERSOURCE`) as well as the winning ones. `amount` is unchanged: the replay over all 1,118 executed 2026 claims did not show the value ladder beating it, and the declared rule was that it moves only if it does. The one signal that was measured and **not** wired is who else wants the man: it repeats the rank (r = 0.05 beyond the bucket) at the price of thirteen spot books a request.
...
**As built** (2026-09-18, `scripts/pickups_backtest.py`): the score is **categories, not composite**. The first cut scored composite and read −3.2 a day at a 20% win rate; that is a measurement of composite rather than of the recommender, which spends points on purpose to win the categories that are close. A move is now scored by replaying the matchup that actually happened with the swap in it, with the daily lineup re-solved: on each day of the window the roster the team really held is read from `daily_lineup_slots`, the swap applied to it, and the ten starting slots filled by the recommender's own seating rule (`stream.seat`, ordered by what each man had averaged *before* that day, so no hindsight sets the lineup); the day's line is the seated men's real box scores. The roster **without** the swap is re-solved the same way, and the two are counted against the opponent's real period totals and subtracted. +1 means the swap flipped one category. The season score is the same replay over the next 30 days across every matchup period the window touches. The baseline is the league's own one-for-one swaps replayed **backwards** and negated, so a real claim and a recommendation are one quantity; `docs/acquirable_value.md`'s +0.57 composite a day is a different quantity and is no longer quoted as a target. Beside them the sweep reports what the recommender **claimed** (the judgement's net) against what it **delivered**, which is the calibration question a win rate cannot answer. Re-solving **both** sides is what keeps lineup optimisation out of a move's credit: scoring a re-solved lineup against the manager's actual starts would pay every move the difference between a well-set lineup and a badly-set one. The gap between the two is reported beside the run as `drift` and enters no score; on 2026 it is +0.02 categories a period (the sign was written down backwards here; the generated write-up has had +0.02 on both runs), so this league's managers set their lineups about as well as the model does. The first category-scored cut capped the added man at the dropped man's started days instead, which scored 60% of named streaming moves at exactly zero -- an empty-day pickup replaces a man who was not starting -- and so could not tune the streaming hurdle at all. Of the two data defects of the first cut, one is gone: the matchup totals leak was fixed in `app/pickups/state.py` on 2026-09-21 (§4.1's fourth pass) and the backtest now measures the product's own totals rather than a substituted copy. The missing 2026 schedule is still worked around by attribute substitution, unchanged.
...
Two reports rather than one `/pickups`, because they answer different questions on different horizons and a caller usually wants one of them; `/week` and `/free-agents` are still to come, and the `TeamWeek` state is visible inside the stream report meanwhile. `today` is a scoring period and defaults to the calendar day turned into one through the stored NBA schedule. Player ids go out as ESPN's, like the rest of the API. An unknown team is a 404 (`TeamDep`); a season with nothing to decide from — not drafted yet, no `pro_team_games`, no roster in either `player_status_snapshots` or the lineup days — was a **409** and is now a **200 with `readiness`** and every number empty (2026-09-25, docs/site.md "Readiness"), because that is a different answer from "no move is worth making" and the page is the one to say it. **The draft is asked first** (`app.inseason.drafted`): before it, ESPN's roster feed shows last season's rosters on every future day, and a lineup row is not a roster. Neither route makes an ESPN request. Both reports are bounded, so both return an object rather than a `Page`.
...
- Sections 1, 2 and 5 are in; 4 waits on the rest-of-season report. A free agent carries his percent owned rather than a value rank, for the same reason.
- **Section 3 is in as built, second pass** (2026-09-18, `app/digest.py`'s `week_plan`): the morning message now carries the day's streaming plan, built from `stream_recommendations` for the scoring period the digest's own clock falls in. The matchup header (period, days left, opponent, expected categories as things stand, adds used of budget), then the days a slot goes empty, then the plan — the moves worth a look, in the order to make them, each on one line in the streaming CLI's words (add X of his games left, drop Y, week + season = net, and the projected record with and without) — or the one line saying nothing clears the bar and how many free agents were weighed. There is no switch: the plan is in the morning digest, which is the pass `scripts/digest.py` sends with no arguments, and never in the `--alert` the later passes send, because an add is not what a player ruled out at 22:30 calls for. **The recommender can never break the digest**: a bye, a day in no matchup period, a season with no stored schedule and any unexpected exception all become one line, and the other three sections are untouched. An unexpected failure is named by its exception *type* only, since an exception's text can carry a query or a connection string. The cap rose from 40 lines to 52 to pay for the section, leaving every other section's budget exactly where it was.
- **Wording, Patrick's rule: the tool generates ideas, managers decide** (2026-09-18, `scripts/stream.py`, `scripts/season.py`, `app/api/pickups.py`, `app/api/schemas.py`, and the digest section above). "Recommended" is gone from everything a human reads: a plan is "worth a look, in this order", and a report with nothing in it says "nothing clears the bar". The hurdle is described as a bar a move clears, never as an instruction to make it. The field names are unchanged — `recommended`, `hurdle` and `clears_hurdle` still say what they always said in the API and in `StreamReport`, because renaming them breaks every caller — so this is a change of voice, not of contract.
- **Both reports are pages as built** (2026-09-18, `app/api/pages.py`, `app/api/static/`, `docs/in_season_pages.md`): the streaming and rest-of-season reports are served as one page each, plus an index of the league, in the season report's light skin over the draft screen's own colour tokens. Two things a played season needs came out of building them, both in `app/pickups/state.py`. **The wire falls back** when a season has no `free_agent_snapshots` at all — which is every season but the one in progress — to `scripts/pickups_backtest.py`'s historical definition (played that scoring period, in nobody's lineup that day), flagged on both reports as `historical_wire` so a page says "historical wire (no snapshots)" rather than "0 free agents evaluated"; a caller that names its own `pool` never reaches it, so the backtest's numbers are unchanged. **FAAB is counted as of the day reported on**: `_faab_spent` summed every executed bid of the season with no day bound, so a report about day 100 charged money spent after it, which is how one 2026 team read $-3 when it had $11. Summed correctly the bid feed still disagrees with ESPN's own `teams.acquisition_budget_spent` for six of fourteen 2026 teams in both directions (+4 to -4), and ESPN's column never exceeds the budget, so the pot is floored at zero and the overshoot carried beside it as `faab_overspent` — never a negative rendered as if a manager could bid it. The routes' readiness guard, which demanded a status snapshot and so refused every played season, now asks for a schedule and a roster from either the snapshots or the stored lineup days.
...
- Caps keep it readable on a phone: eight roster events, ten wire ones, eight status lines, three empty days, and 52 lines in all.
- **Both trailing counts have a far end** (2026-09-21, `app/digest.py`): the churn line's fortnight (`adds_in_window`) and the league section's day of wire traffic (`league_section`) asked only for rows processed since `now - window`, never for rows older than `now`. A window with one end is not a trailing window but everything since that moment, which live reads right and on a replay reads the rest of the season: the in-season rehearsal's day-80 digest said 83 adds in the last fortnight where 15 had been made, and 609 moves on the wire in the last day where nine had (docs/inseason_rehearsal.md, finding 3).
...
- **Email as a third shape** (2026-09-18, `app/notify.py`'s `send_email` and `deliver`): plain SMTP through the standard library — `smtplib` and `email.message.EmailMessage`, no provider library — so any transactional provider's endpoint works: Postmark, Resend, SES, Fastmail, a relay on the box. Port 465 is implicit TLS and anything else is STARTTLS, so the body and the login are encrypted either way; a user without a password is treated as no credentials rather than half a login. The settings are `FCP_EMAIL_TO` (one address or several, comma separated), `FCP_EMAIL_FROM`, `FCP_SMTP_HOST`, `FCP_SMTP_PORT` (587 by default), `FCP_SMTP_USER` and `FCP_SMTP_PASSWORD`; all are optional and blank means unset, like the digest's own. Email is configured when the host, the sender and at least one recipient are set. **`FCP_SMTP_PASSWORD` lives only in `.env` on the VPS** — not in a unit file, not in a shell history, not in this repository — and is never logged, never printed and never part of an error message; the test that pins that is `test_a_failed_email_never_carries_the_password`. The subject is the message's title (the pass and its date), the body the same plain text.
- **Every configured channel gets the digest.** `deliver` sends to the URL channel if it is set and to email if it is set, catching each one separately, and returns what each did: one channel failing does not stop the other, and `scripts/digest.py` prints both outcomes in its log line. Events are marked `notified_at` once *any* channel has delivered — the manager has the news — while the script still exits 1 if any channel failed, so a broken channel shows up in the unit instead of dying quietly. With no channel configured at all it prints and marks nothing, as before.
- `scripts/notify_test.py` sends a one-line test to every configured channel, or to one of them with `--email` / `--url`, and prints the host, port, sender and recipients it used. For running on the VPS after editing `.env`, so a misconfigured channel is found then rather than at 09:00 the next morning.
...
**As built, fifth pass** (2026-09-22, `app/calibration.py`, docs/intake.md): the three bars and the two wire measurements are no longer constants. Each is a row in `league_calibrations`, per league, with where it came from: the league's own manager chose it, or it was measured on that league's own backtest, or it is the pool of leagues shaped like it, or it is the default -- which is the constant, unchanged, still `STREAM_HURDLE = 0.20` and `TYPICAL_PICKUP = 0.06` in the modules that used to own them. **Full Court Press prints exactly what it printed**: migration `0027` seeds it with `owner` rows for the three bars Patrick chose, dated and with his reasons on them, and `measured` rows for the two wire numbers, every value the constant it replaces. The recommenders take them as parameters (`hurdle`, `hurdle_paid`, `hurdle_free`, `floor`, `opened`) whose defaults are the constants, so `test_pickups_judge.py`, `test_pickups_season.py` and `test_pickups_stream.py` pass untouched again, and the routes, the digest and the CLIs pass the league's own. The bar's provenance travels with the report (`hurdle_source`, `hurdle_note`) and is printed under it the way a projection's `source_note` is, because a bar labels and a bar whose provenance is hidden is a bar asking to be trusted. A second league's bars are fitted by `intake_hurdles`, the sweep of section 4 run on its history; where the tuning rule picks nothing -- which is what it does on the streaming side of this league every time -- the row records the grid and the sample, the page says the sweep chose nothing, and the fallback carries on.
