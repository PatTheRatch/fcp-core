...
| `draft_board(league_id, season, team_id, source?)` | the pre-auction plan as the plan page reads it (docs/draft_plan.md): the ladder (the model's and his), the cap, the forty dearest men with the model's going price and ceiling, what the build read once his own going price and ceiling were applied, and his marks, side by side; his marked men; the model's lists with their rules, twelve men each; his must-have men and what fixing them in costs; the best roster at his prices. `sources` lists every pool he may plan on, exactly as the page's SOURCES and chooser do (BBM for its owner, ESPN, his uploads, his composites with their recipes); `source` names one as `bbm`, `espn`, `upload:<set id>`, `composite:<set id>` or by its own name, and a name he has no source by is refused with the ones he has. Gated on the pool as the page is: a plan on Basketball Monster, or on a composite with BBM in it, only for the owner of that source, else `withheld` and the pools he may plan on; `building` while a cold plan is worked out; after the auction, when it was held. Provenance names the pool and its capture date. It never bids or nominates | 28k chars, ~7,000 (the 2027 BBM plan) |
